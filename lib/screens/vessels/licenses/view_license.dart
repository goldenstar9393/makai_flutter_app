...
class _ViewLicenseState extends State<ViewLicense> {
  ...
  void initState() {
    ...
    dobTEC.text =
        DateFormat('MMMM dd, yyyy').format(widget.license.issueDate!.toDate());
    buildDateTEC.text =
        DateFormat('MMMM dd, yyyy').format(widget.license.dob!.toDate());
    expiryDateTEC.text =
        DateFormat('MMMM dd, yyyy').format(widget.license.expiryDate!.toDate());
        ...
        child: Form(
          ...
          child: Column(
            children: [
                  ...
                  itemBuilder: (context, i) {
                    return InkWell(
                      onTap: () => Get.to(() => ViewImages(
                          index: i, images: widget.license.licenses!)),
                      child: Padding(
                        padding: const EdgeInsets.only(right: 10),
                        child: CachedImage(
                            height: 80,
                            roundedCorners: true,
                            url: widget.license.licenses![i]),
                      ),
                    );
                  },
                ),
              ...
              CustomTextField(
                  controller:
                      TextEditingController(text: certificateType.value),
                  label: 'License Type *',
                  enabled: false),
              CustomTextField(
              ...
              if (MY_ROLE == VESSEL_CAPTAIN || MY_ROLE == VESSEL_OWNER)
                CustomButton(
                  text: 'Modify License',
                  function: () =>
                      Get.off(() => EditLicense(license: widget.license)),
                ),
            ],
          ),
        ),
